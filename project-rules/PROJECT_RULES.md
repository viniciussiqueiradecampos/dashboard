...
## 🔹 Pre-flight Check (Mental)
Before generating code:
- Confirm adherence to User and Project Rules.
- Ensure fluid layout and mobile-first approach.
- Prioritize semantic variables; fallback to primitives.
- Ensure parent components are fully responsive and fluid.
...
- Wide / 4K: `max-w-[1600px]` (evita linhas longas demais em dashboards financeiros)
...
- **Desktop (≥1280px):**
  ...
  - Possui dois estados: Expanded (larga, com texto) e Collapsed (estreita, apenas ícones)
...
- Contém: Botão de menu (abre drawer), Ações principais (ex: nova transação)
...
- Inputs no mobile: Altura mínima 48px, Font-size mínimo 16px
...
1. **Variável SEMÂNTICA aplicada no Figma?** → Usar diretamente (ex: `--color-primary`)
2. **Variável PRIMITIVA aplicada no Figma?** → Usar diretamente (ex: `--gray-900`)
3. **Valor local (hex, px, rem)?** → **CONVERSÃO INTELIGENTE:**
   - **CORES HEX:** Comparar visualmente com primitivas e escolher a MAIS PRÓXIMA. NUNCA inventar novos tokens.
   - **ESPAÇAMENTOS:** Arredondar para token da escala existente. NUNCA usar valores quebrados.
   - **TIPOGRAFIA:** Mapear peso e tamanho para escala tipográfica.
4. **Valor Hardcoded?** → NUNCA.
...
🔨 BUILD STATUS
✅ Sucesso (ou falha com correções)
