...
export interface MenuItemConfig {
    id: string;
    name: string;
    originalName: string; // Keep track of the default name for translations
    enabled: boolean;
}
...
interface SettingsContextType {
    ...
    renameMenuItem: (id: string, newName: string) => void;
    isMasterUser: boolean;
    formatCurrency: (value: number) => string;
}

const DEFAULT_MENU_ITEMS: MenuItemConfig[] = [
    { id: 'dashboard', name: 'Dashboard', originalName: 'Dashboard', enabled: true },
    { id: 'transactions', name: 'Transações', originalName: 'Transações', enabled: true },
    { id: 'cards', name: 'Cartões', originalName: 'Cartões', enabled: true },
    { id: 'goals', name: 'Metas', originalName: 'Metas', enabled: true },
    { id: 'profile', name: 'Perfil', originalName: 'Perfil', enabled: true },
];
...
export function SettingsProvider({ children }: { children: React.ReactNode }) {
            ...
            try {
                ...
                if (settings) {
                    ...
                    if (settings.enabled_menus && Array.isArray(settings.enabled_menus)) {
                        ...
                        const customNames = settings.custom_menu_names || {};
    ...
    const renameMenuItem = useCallback(async (id: string, newName: string) => {
        let updatedItems: MenuItemConfig[] = [];
        ...
        if (user) {
            const customNames = updatedItems.reduce((acc, item) => {
            ...
            try {
                const { error } = await supabase
                    ...
                    .upsert({
                        user_id: user.id,
                        custom_menu_names: customNames,
                        updated_at: new Date().toISOString()
                    } as any);

                if (error) console.error('Error saving menu names to DB:', error);
            } catch (err) {
                console.error('Error saving menu names:', err);
            }
        }
    }, [user]);
